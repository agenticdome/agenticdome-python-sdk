...
execute tools, or call Microsoft AI Foundry. The framework option is a label
and guide selector, not a Foundry integration test.
...
Run `agenticdome-demo --framework foundry --scenario both --live` to obtain
real tenant-engine decisions for those fixed inputs. That checks the assigned
sidecar, not Foundry attachment; the code below attaches the adapter to the
