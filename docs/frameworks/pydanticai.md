...
execute tools, or instantiate PydanticAI. The framework option is a label and
guide selector, not a PydanticAI integration test.
...
Run `agenticdome-demo --framework pydanticai --scenario both --live` to obtain
real tenant-engine decisions for those fixed inputs. That checks the assigned
sidecar, not PydanticAI attachment; the code below attaches the adapter to the
