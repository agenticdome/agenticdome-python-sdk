...
execute tools, or instantiate LlamaIndex. The framework option is a label and
guide selector, not a LlamaIndex integration test.
...
Run `agenticdome-demo --framework llamaindex --scenario both --live` to obtain
real tenant-engine decisions for those fixed inputs. That checks the assigned
sidecar, not LlamaIndex attachment; the code below attaches the adapter to the
