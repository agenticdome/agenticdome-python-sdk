...
execute tools, or instantiate OpenAI Agents SDK. The framework option is a
label and guide selector, not an OpenAI Agents integration test.
...
Run `agenticdome-demo --framework openai-agents --scenario both --live` to
obtain real tenant-engine decisions for those fixed inputs. That checks the
assigned sidecar, not OpenAI Agents attachment; the code below attaches the
adapter to the real application boundary.
