...
execute tools, or instantiate Google ADK. The framework option is a label and
guide selector, not a Google ADK integration test.
...
Run `agenticdome-demo --framework google-adk --scenario both --live` to obtain
real tenant-engine decisions for those fixed inputs. That checks the assigned
sidecar, not Google ADK attachment; the code below attaches the adapter to the
