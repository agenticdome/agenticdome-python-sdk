...
CONFIG = FirewallConfig(
    ...
    redis_key_prefix=os.getenv("AGENTICDOME_REDIS_KEY_PREFIX", "AgenticDome:crewai:handoff"),
...
if CONFIG.api_base and CONFIG.api_key and CONFIG.tenant_id:
    try:
        ...
        # Compatibility for clients constructed as AgentGuardClient(api_base, options)
...
def _build_token_store() -> DecisionTokenStore:
    if CONFIG.redis_url:
        try:
            ...
            logger.warning(
                "Redis dependency missing. Install with: pip install redis. "
    ...
    try:
        ...
        if _is_handoff_tool(tool_name):
            ...
            response = _client_call(
                ...
                tool_platform=str(tool_args.get("tool_platform") or CONFIG.default_tool_platform),
...
def AgenticDome_before_llm_call(context: Any) -> bool:
    ...
    try:
        ...
        response = _client_call(
            ("guardrail_validate", "guardrailValidate"),
            text=str(prompt) or "[empty prompt]",
            agent_id=agent_id,
            direction="input",
            session_id=session_id,
            platform=CONFIG.platform,
