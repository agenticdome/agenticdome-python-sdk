...
| :--- | :--- | :--- | :--- | :--- |
| CrewAI | Application bootstrap, before any `Crew` is constructed | `import agenticdome_sdk.crewai` or `AgenticDomeCrewAIFirewall().attach(crew)` | Crew prompts and every sensitive local tool | [CrewAI](../README.md#crewai) |
| PydanticAI | Module that constructs each `Agent` and registers its tools | `firewall.install_native_hooks(agent)` plus `@firewall.secure_tool(...)` | Direct calls to the undecorated tool implementation | [PydanticAI](../README.md#pydanticai) |
| LangGraph / LangChain | Graph/agent builder before `compile()` or `create_agent()` returns | Firewall nodes/wrappers or `firewall.as_langchain_middleware(...)` | Graph edges or tool nodes that skip the secured transition | [LangGraph](../README.md#langgraph) |
| Microsoft Agent Framework | Agent/workflow factory and tool registration module | `firewall.install_on_agent(agent)` or `firewall.run_agent_securely(...)` | Raw tool handlers and direct runs outside the middleware | [Microsoft Agent Framework](../README.md#microsoft-agent-framework) |
| Microsoft AutoGen | AgentChat team/Core runtime construction | `firewall.wrap_team(team, session_id=...)` or the documented intervention handler | Direct `team.run()` calls and uninspected Core tool events | [Microsoft AutoGen](../README.md#microsoft-autogen) |
| Microsoft AI Foundry | Foundry client/run and local tool-executor construction | `firewall.install_on_client(client)`, `run_secure(...)` or `wrap_tool_executor(...)` | Local function execution outside the secured executor | [Microsoft AI Foundry](../README.md#microsoft-ai-foundry) |
| OpenAI Agents SDK | Module constructing `Agent`, `Runner` and `@function_tool` functions | `firewall.run_agent_securely(...)` and `firewall.wrap_tool_handler(...)` | Calling `Runner` or raw side-effecting functions directly | [OpenAI Agents SDK](../README.md#openai-agents-sdk) |
| Claude Agent SDK | Module constructing `ClaudeAgentOptions`, client or SDK MCP tools | `firewall.install_on_options(options)` plus `run_client_securely(...)` or `secure_query(...)` | A client/query path using options without the installed hooks | [Claude Agent SDK](../README.md#claude-agent-sdk) |
| Hugging Face smolagents | Module constructing `CodeAgent`/`ToolCallingAgent` and native tools | `firewall.run_agent_securely(...)` or `firewall.attach_firewall(...)` | Direct executors, tools or managed agents outside the attached agent | [smolagents](../README.md#hugging-face-smolagents) |
| Agno | Agent, Team, Workflow or AgentOS component factory | `firewall.attach_firewall(agent_or_team)` | Components created after bootstrap without hooks; raw high-risk tools | [Agno](../README.md#agno) |
| Google ADK | `LlmAgent` construction or central plugin registration | `**firewall.build_callback_kwargs()` or `firewall.install_on_agent(agent)` | Tools not exposed through the secured callbacks/wrappers | [Google ADK](../README.md#google-adk) |
| LlamaIndex | Tool, query engine, retriever and callback assembly module | `to_function_tool(...)`, `wrap_query_engine(...)` or `run_query_securely(...)` | Direct query/retrieval/tool paths outside the selected wrapper | [LlamaIndex](../README.md#llamaindex) |
| AWS Bedrock | Code that calls boto3 runtime/agent clients and action-group handlers | `converse_securely(...)`, `invoke_model_securely(...)` or the matching wrapper | Direct boto3 calls and unwrapped action-group/local tool handlers | [AWS Bedrock](../README.md#aws-bedrock) |
| MCP host / gateway | The single JSON-RPC forwarding function | `firewall.forward_with_firewall(...)` or `preflight_request(...)` | Any alternate route that forwards `tools/call` directly | [MCP host / gateway](../README.md#mcp-host--gateway) |
| Custom Python | Your prompt handler, tool executor, delegation router and response boundary | `guardrail_validate()` before execution and `mesh_validate()` before return | Calling the real function before checking the verdict | [Custom Python](../README.md#core-sdk-client-custom-runtimes) |
